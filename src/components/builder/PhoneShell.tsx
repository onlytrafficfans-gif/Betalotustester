...
// Supports phone, tablet, and desktop viewports with accurate sizing
...
export function PhoneShell({ device, children }: PhoneShellProps) {
  ...
  return (
    <div
      className={cn('relative border-black bg-black overflow-hidden shadow-2xl', config.frameClass)}
      style={{
        width: config.width,
    ...
    >
      ...
      {/* Content */}
      <div className={cn('h-full w-full overflow-hidden bg-[#050505] relative', config.innerClass)}>
        {children}
      </div>
    </div>
  );
