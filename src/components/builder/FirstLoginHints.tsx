...
export function FirstLoginHints() {
  ...
  useEffect(() => {
    ...
    supabase.from('user_profiles').select('has_seen_hints').eq('id', userId).single().then(({ data }) => {
      if (!data?.has_seen_hints) setShow(true);
      ...
      <div className="w-full max-w-sm mx-4 bg-[#141414] border border-white/5 rounded-2xl shadow-2xl overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/5">
          <div className="flex items-center gap-2">
            <div className="w-5 h-5 rounded overflow-hidden"><img src="/logo-lotus.png" alt="LOTUS" className="w-full h-full object-cover" /></div>
            <span className="text-xs font-semibold text-white/70">Welcome to LOTUS</span>
          </div>
        ...
        <div className="px-5 py-6">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-10 h-10 rounded-xl bg-lotus-400/10 flex items-center justify-center">{hint.icon}</div>
            <h3 className="text-sm font-semibold text-white/80">{hint.title}</h3>
          </div>
        ...
        <div className="px-5 pb-5">
          <div className="flex items-center justify-center gap-1.5 mb-4">
            {HINTS.map((_, i) => (
              <button key={i} onClick={() => setStep(i)} className={`w-1.5 h-1.5 rounded-full transition-all ${i === step ? 'bg-lotus-400 w-4' : 'bg-white/10 hover:bg-white/20'}`} />
            ))}
          </div>
          <div className="flex items-center gap-2">
